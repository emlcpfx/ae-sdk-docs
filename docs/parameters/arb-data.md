...
*Tags: `arb-data`, `render-loop`, `threading`, `ui`*
...
*Tags: `arb-data`, `params`, `render-loop`, `threading`*
...
*Tags: `aegp`, `arb-data`, `params`, `scripting`, `ui`*
...
*Tags: `aegp`, `arb-data`, `params`, `scripting`*
...
*Tags: `arb-data`, `params`, `scripting`, `ui`*
...
*Tags: `aegp`, `arb-data`, `scripting`, `ui`*
...
*Tags: `arb-data`, `params`, `scripting`*
...
*Tags: `arb-data`, `expressions`, `params`, `scripting`, `ui`*
...
*Tags: `aegp`, `arb-data`, `cross-platform`, `params`, `scripting`, `ui`*
...
*Tags: `arb-data`, `layer-checkout`, `params`, `ui`*
...
*Tags: `aegp`, `arb-data`, `render-loop`, `ui`*
...
*Tags: `aegp`, `arb-data`, `params`, `sdk`, `ui`*

---

### Should PF_Arbitrary_COPY_FUNC reallocate or reuse the destination handle?

Reuse if non-null, allocate only if null. AE pre-allocates `*dst_arbPH` via your NEW_FUNC, so it is always the correct size. **Never dispose `*dst_hp` inside COPY_FUNC** — AE owns the handle lifecycle and tracks it internally. Disposing it creates a dangling pointer that AE will later double-free via DISPOSE_FUNC, causing heap corruption (`c0000374`). The crash often appears in an unrelated code path (UI redraw, param seek) because the corruption is detected lazily by the heap manager:

```cpp
// WRONG — double-free! AE still tracks the old handle
if (*dst_hp) { PF_DISPOSE_HANDLE(*dst_hp); *dst_hp = NULL; }

// CORRECT — allocate only if null, reuse otherwise
case PF_Arbitrary_COPY_FUNC: {
    PF_Handle src_h = extra->u.copy_func_params.src_arbH;
    PF_Handle* dst_hp = extra->u.copy_func_params.dst_arbPH;
    if (!src_h) break;
    if (!*dst_hp) {
        *dst_hp = PF_NEW_HANDLE(sizeof(MyArbData));
        if (!*dst_hp) return PF_Err_OUT_OF_MEMORY;
    }
    auto* src = reinterpret_cast<MyArbData*>(PF_LOCK_HANDLE(src_h));
    auto* dst = reinterpret_cast<MyArbData*>(PF_LOCK_HANDLE(*dst_hp));
    if (src && dst) memcpy(dst, src, sizeof(MyArbData));
    if (src) PF_UNLOCK_HANDLE(src_h);
    if (dst) PF_UNLOCK_HANDLE(*dst_hp);
    break;
}
```

Apply the same pattern to `INTERP_FUNC`'s `interpPH` destination — allocate only if null, never dispose.

*Tags: `arb-data`, `copy-func`, `heap-corruption`, `memory`, `pf-handle`*

---

### What is the correct pattern for PF_Arbitrary_DISPOSE_FUNC?

Keep it simple — just null-check and dispose. AE gives you the handle to free; always free it:

```cpp
case PF_Arbitrary_DISPOSE_FUNC: {
    PF_Handle arbH = extra->u.dispose_func_params.arbH;
    if (arbH) PF_DISPOSE_HANDLE(arbH);
    break;
}
```

Do NOT lock the handle to check magic values before disposing. Locking a handle that's already been freed (e.g., due to a bug elsewhere) is use-after-free. Skipping the dispose causes a memory leak and can confuse AE's handle tracking. If DISPOSE_FUNC crashes with heap corruption, the root cause is upstream — look for double-frees in COPY_FUNC (disposing `*dst_hp`) or in code that manually disposes handles after `AEGP_SetAddKeyframe`.

*Tags: `arb-data`, `dispose-func`, `memory`, `pf-handle`*

---

### Does AEGP_SetAddKeyframe take ownership of arb handles?

Yes. When you call `AEGP_SetAddKeyframe(akH, key_idx, &sv)` with `sv.val.arbH = your_handle`, AE copies the arb data via your COPY_FUNC callback, then disposes the original via your DISPOSE_FUNC. Do NOT dispose the handle yourself afterward — that causes a double-free and heap corruption:

```cpp
// CORRECT
PF_Handle arb_h = PF_NEW_HANDLE(sizeof(MyArbData));
auto* arb = reinterpret_cast<MyArbData*>(PF_LOCK_HANDLE(arb_h));
arb->magic = MY_ARB_MAGIC;
// ... fill data ...
PF_UNLOCK_HANDLE(arb_h);
...
// DO NOT PF_DISPOSE_HANDLE(arb_h) — AE owns it now

// WRONG — storing handles for later disposal
arb_handles.push_back(arb_h);  // Don't do this
// ...
for (auto h : arb_handles) PF_DISPOSE_HANDLE(h);  // Double-free!
```

*Tags: `aegp`, `arb-data`, `double-free`, `heap-corruption`, `keyframes`, `memory`*

---

### How do I safely dereference a locked arb handle that might be corrupted?

`PF_LOCK_HANDLE` can return a non-null but invalid pointer when the handle has been freed or corrupted (e.g., from a prior double-free or heap overflow). A bare `if (ptr)` null check passes, then dereferencing crashes with `c0000005` (access violation). Use a pointer validation function:

```cpp
static inline bool ArbPtrValid(const void* p) {
    auto addr = reinterpret_cast<uintptr_t>(p);
    return addr > 0xFFFF                    // Above 64KB guard region
        && addr < 0x00007FFFFFFFFFFF        // Within user address space (64-bit)
        && (addr & 7) == 0;                 // 8-byte aligned (heap guarantee)
}

// Use in all arb callbacks:
auto* src = reinterpret_cast<MyArbData*>(PF_LOCK_HANDLE(src_h));
if (!ArbPtrValid(src) || src->magic != MY_ARB_MAGIC) {
    if (ArbPtrValid(src)) PF_UNLOCK_HANDLE(src_h);
    break;  // Graceful no-op instead of crash
}
```

Apply this in COPY_FUNC, FLATTEN_FUNC, UNFLATTEN_FUNC, INTERP_FUNC, and COMPARE_FUNC — anywhere you lock and dereference an arb handle.

*Tags: `arb-data`, `crash`, `memory`, `pf-handle`, `pointer-validation`*
