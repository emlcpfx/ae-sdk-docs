...
> 115 Q&As · source: AE plugin dev community Discord
...
*Tags: `adobe`, `legacy`, `pipl`, `sdk-history`*
...
*Tags: `aegp`, `pipl`, `plugin-architecture`*
...
*Tags: `apple-silicon`, `build`, `macos`, `pipl`*
...
*Tags: `apple-silicon`, `build`, `cross-platform`, `macos`, `pipl`*
...
*Tags: `build`, `cross-platform`, `macos`, `pipl`, `windows`*
...
*Tags: `aegp`, `macos`, `pipl`, `sdk`, `windows`*
...
*Tags: `build`, `pipl`, `sdk`, `visual-studio`, `windows`*
...
*Tags: `build`, `cross-platform`, `macos`, `pipl`, `sdk`, `windows`*
...
*Tags: `aegp`, `layer-checkout`, `pipl`, `scripting`*

---

### AE refuses to load my plugin with "version mismatch" and "outflags mismatch" errors. How do I fix the PiPL version and flag encoding?

Two separate issues with the same symptom (plugin refuses to load with error code 80000 or similar):

**Version encoding**: Do not hand-roll the PiPL version integer — replicate `PF_VERSION` exactly. A common mistake is `(major<<16)+(minor<<8)+patch`, which AE decodes incorrectly. The actual bit layout used by `PF_VERSION` in `AE_Effect.h`:

```cmake
# CMake equivalent of PF_VERSION(major, minor, bug, stage, build)
# vers_high=bits 26-29, vers_low=bits 19-21, subvers=bits 15-18,
# bugfix=bits 11-14, stage=bits 9-10, build=bits 0-8
math(EXPR PLUGIN_VERSION
    "(${MAJOR}/8)*67108864 + (${MAJOR}%8)*524288 + (${MINOR}%16)*32768 + (${PATCH}%16)*2048")
```

For v1.0.0 this produces 524288, which AE decodes as "1.0". The naive `(1<<16)=65536` decodes as "0.2".

**Flag superset mismatch**: The PiPL `AE_Effect_Flags` and `AE_Effect_Flags_2` must be a **superset** of whatever `GlobalSetup` sets. AE compares them at load time and errors on any mismatch. The solution is to set the full superset of all flags the effect could ever need in both PiPL and `GlobalSetup`, then use `PF_Cmd_QUERY_DYNAMIC_FLAGS` to clear flags that don't apply at runtime:

```cpp
// GlobalSetup: always set the full superset
...
// QueryDynamicFlags: clear flags that don't apply to THIS instance
case PF_Cmd_QUERY_DYNAMIC_FLAGS:
    out_data->out_flags  = FULL_SUPERSET;
    out_data->out_flags2 = FULL_SUPERSET;
    if (!has_temporal) out_data->out_flags &= ~PF_OutFlag_WIDE_TIME_INPUT;
    if (!has_gpu)      out_data->out_flags2 &= ~PF_OutFlag2_SUPPORTS_GPU_RENDER_F32;
    break;
```

*Tags: `pipl`, `version`, `outflags`, `global-setup`, `query-dynamic-flags`, `plugin-load`*

---

### What does error 25::16 mean when loading an AE plugin?

Error 25::16 is AE's generic "plugin refused to load" code, most often triggered by a PiPL/GlobalSetup mismatch. Three variants appear in the error dialog:

- **"effect has version mismatch. Code version is X and PiPL version is Y."** — The version integer in PiPL doesn't match what GlobalSetup reports. See the PF_VERSION encoding question above.
- **"effect has global outflags mismatch. Code flags are X and PiPL flags are Y."** — `out_flags` in GlobalSetup differs from PiPL `AE_Effect_Flags`.
- **"effect has global outflags2 mismatch."** — Same for `out_flags2`.

All three require matching the PiPL resource exactly to GlobalSetup. Clean-rebuild after any PiPL or flag change — the `.rc` file generated from the `.r` PiPL resource is not always rebuilt incrementally.

*Tags: `pipl`, `error-25-16`, `outflags`, `version`, `plugin-load`*
