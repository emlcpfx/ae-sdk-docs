...
> 136 Q&As · source: AE plugin dev community Discord
...
*Tags: `layer-checkout`, `params`, `sdk`*
...
### After Effects becomes progressively slower and eventually crashes when using my plugin with a layer parameter. What causes this?

Every `checkout_layer_pixels()` call must be paired with a `checkin_layer_pixels()` call, and every `PF_CHECKOUT_PARAM` must be paired with `PF_CHECKIN_PARAM`. Missing check-ins hold pixel buffers that AE cannot free, causing continuous memory growth.

The safe pattern is to track all checked-out handles and flush them unconditionally after render completes — even if render throws:

```cpp
// Track during render
std::vector<int> checked_out_layer_indices;
std::map<int, PF_ParamDef> checked_out_params;

// On each get_layer() call:
extra->cb->checkout_layer_pixels(effect_ref, layer_idx, &world);
checked_out_layer_indices.push_back(layer_idx);

// On each param read — cache to avoid repeated checkout:
auto it = checked_out_params.find(param_idx);
if (it == checked_out_params.end()) {
    PF_CHECKOUT_PARAM(in_data, param_idx, time, step, scale, &def);
    checked_out_params[param_idx] = def;
}

// After render() returns:
for (int idx : checked_out_layer_indices)
    extra->cb->checkin_layer_pixels(effect_ref, idx);
for (auto& [idx, def] : checked_out_params)
    PF_CHECKIN_PARAM(in_data, &def);
```

Treat checkout/checkin as malloc/free — every checkout path needs a corresponding checkin, and caching params within a single render call avoids redundant checkout/checkin cycles.

*Tags: `layer-checkout`, `checkin`, `memory-leak`, `smart-render`, `params`*
